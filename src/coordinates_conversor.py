...
def getDecFromDeg(deg):
	...
	mm = int((deg - hh)*60)
	ss = ((deg - hh)*60 - mm )*60
	return sign * hh, mm,("%.2f"%ss).zfill(5)
...
def deg2hour(ra_deg, dec_deg, sep=":"):
    ...
    ra, dec= "%.2d:%.2d:%s"%(ra_hour), sign+"%.2d:%.2d:%s"%(dec_hour)
    return ra.replace(":", sep), dec.replace(":", sep).replace("--","-")
